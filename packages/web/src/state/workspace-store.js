...
import { listWorkflows, createWorkflow as createWorkflowApi, getWorkflowDetail, updateWorkflowState, deleteWorkflow as deleteWorkflowApi, saveWorkflowVersion as saveWorkflowVersionApi, restoreWorkflowVersion as restoreWorkflowVersionApi, renameWorkflowVersion as renameWorkflowVersionApi, deleteWorkflowVersion as deleteWorkflowVersionApi } from "../services/workflow-api";
const deriveNameFromDocument = (document) => {
...
const flushPendingUpdate = async (get, set) => {
    ...
    try {
        ...
        set((state) => {
            const workflows = state.workflows.map((item) => item.id === mapped.id
                ? {
                    ...item,
                    name: mapped.name,
...
export const useWorkspaceStore = createWithEqualityFn()((set, get) => ({
    workflows: [],
    ...
    selectWorkflow: async ({ id }) => {
        ...
        try {
            ...
            const mapped = mapDetail(detail);
            set((prev) => {
                const workflows = prev.workflows.map((item) => item.id === mapped.id
                    ? {
                        ...item,
                        name: mapped.name,
                        status: mapped.status,
    ...
    updateActiveWorkflow: ({ document, code, type, name, status }) => {
        ...
        set((prev) => {
            ...
            const workflows = prev.workflows.map((item) => item.id === workflowId
                ? {
                    ...item,
                    name: derivedName,
                    updatedAt: updatedAtIso,
    ...
    renameWorkflowVersion: async ({ workflowId, versionId, name }) => {
        try {
            ...
            set((prev) => {
                ...
                const versions = prev.activeWorkflow.versions.map((item) => item.id === version.id
                    ? {
                        ...item,
                        name: version.name,
...
export const useActiveWorkflow = () => {
    return useWorkspaceStore((state) => state.activeWorkflow, (a, b) => a?.id === b?.id &&
