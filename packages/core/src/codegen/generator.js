...
const parseExpressionSafely = (code) => {
    try {
...
const parseForInitializer = (code) => {
    if (code.trim() === "") {
        return null;
    }
    const ast = recastParse(code, recastOptions);
...
const updateOperationToBinaryOperator = {
    add: "+",
    subtract: "-",
    multiply: "*",
    divide: "/",
...
const updateOperationToCompoundOperator = {
    add: "+=",
    subtract: "-=",
    multiply: "*=",
    divide: "/=",
...
const parameterIdentifiers = (parameters) => {
    if (!parameters) {
        return [];
    }
    return parameters
...
const blockToStatement = ({ block, document }) => {
    switch (block.kind) {
        case "function-declaration": {
            const name = typeof block.data.identifier === "string" ? block.data.identifier : "anonymous";
            const params = parameterIdentifiers(block.data.parameters);
        ...
        case "array-push": {
            ...
            const call = t.callExpression(t.memberExpression(t.identifier(arrayName), t.identifier("push")), [valueExpression]);
            if (storeResult) {
        ...
        case "press-call": {
            ...
            const args = [parseExpressionSafely(key)];
        ...
        case "scroll-call": {
            ...
            return [
                t.expressionStatement(t.callExpression(t.identifier("scroll"), [originExpression, t.stringLiteral(direction), amount]))
            ];
        }
        ...
        case "file-reader-call": {
            ...
            const declaration = t.variableDeclaration("let", [
                t.variableDeclarator(t.identifier(assignTo), t.callExpression(t.identifier("fileReader"), [pathsExpression]))
            ]);
            return [declaration];
        }
        case "log-call": {
        ...
        case "function-call": {
            const functionName = typeof block.data.functionName === "string" && block.data.functionName.trim() !== ""
        ...
        case "open-call": {
            ...
            const args = [parseExpressionSafely(appName)];
        ...
        case "ai-call": {
            ...
            const args = [parseExpressionSafely(prompt)];
            const properties = [];
        ...
        case "vision-call": {
            ...
            const schemaCode = typeof block.data.schema === "string" ? block.data.schema : "";
            const args = [targetExpression, parseExpressionSafely(prompt)];
            const properties = [];
            if (format) {
                properties.push(t.objectProperty(t.identifier("format"), t.stringLiteral(format)));
            }
        ...
        case "screenshot-call": {
            ...
            const args = targetCode ? [parseExpression(targetCode)] : [];
...
export const generateCode = (document) => {
